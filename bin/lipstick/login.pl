...
#  $form->{favicon} = "sql-ledger.ico";
  ...
  $form->header;
    ...
    $content = qq|
        <form class="form-signin" method=post name=main action=$form->{script} >
            ...
            <button class="btn btn-lg btn-primary btn-block" name="action" value="|.$locale->text('Login').qq|" type="submit">|.$locale->text('Login').qq|</button>
            |.$form->hide_form(qw(js path)).qq|
        </form>|;
    ...
    my $vars = {
        titlebar  => $form->{titlebar},
        stylesheet => $form->{stylesheet},
        content => $content,
        js => $js,
    };

    $template->process($form->{adminlte_template}, $vars)
